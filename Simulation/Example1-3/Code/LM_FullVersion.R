...
simu1 <- function(kk,pp,nn,rat){
  ...
  prob <- 0.2
  kk <- function(n){
    #u <- rbinom(n=n,s=1,p=0.4)
    return (2*c((log(200)/sqrt(200)) + (abs(rnorm(sd=1,n=n)))))
  }
  #beta = c(c(1,0.8,0.6),rep(0,15),c(-0.6,-0.8),rep(0,980))
  beta = c(1*kk(3),rep(0,15),-1*kk(2),rep(0,980))
  y <- s %*% beta + rnorm(sd=1,n=200)
  l = floor(y-rpois(2,n=200))
  r = floor(y+2*rchisq(n=200,df=3)+1)
  tot <- prob*nn
...
write.csv(res1,"Full_res1.csv")
write.csv(res2,"Full_res2.csv")
write.csv(res4,"Full_res4.csv")
write.csv(res5,"Full_res5.csv")
write.csv(res7,"Full_res7.csv")
write.csv(res8,"Full_res8.csv")
write.csv(res10,"Full_res10.csv")
write.csv(res11,"Full_res11.csv")
